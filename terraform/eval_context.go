...
type EvalContext interface {
	...
	EvaluateExpr(hcl.Expression, *Resource) (cty.Value, tfdiags.Diagnostics)
	...
	InterpolateProvider(*configs.Provider, *configschema.Block, *Resource) (cty.Value, tfdiags.Diagnostics)

	// SetVariables sets the variables for a module call within
	// this context with the name n. This function call is additive:
	// the second parameter is merged with any previous call.
	SetVariables(string, map[string]cty.Value)
